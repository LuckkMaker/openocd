...
#define APM32_FLASH_LPMR_B0    0x40011024
#define APM32_FLASH_TROR_B0    0x40011028

/* option byte location */

#define APM32_OB_RDP		0x20400
#define APM32_OB_USER		0x20402
#define APM32_OB_DATA0		0x20404
#define APM32_OB_DATA1		0x20406
#define APM32_OB_WRP0		0x20408
#define APM32_OB_WRP1		0x2040A
#define APM32_OB_WRP2		0x2040C
#define APM32_OB_WRP3		0x2040E
#define APM32_OB_AFR		0x20410
#define APM32_OB_HXTCNT		0x20414
#define APM32_OB_CLKOPT		0x20416
...
#define OPT_RDWDGSW		8
#define OPT_RDRSTSTOP	9
#define OPT_RDRSTSTDBY	10
...
static int apm32x_erase_options(struct flash_bank *bank)
{
	...
	retval = target_write_u32(target, APM32_FLASH_OBKEYR_B0, KEY1);
	if (retval != ERROR_OK)
		goto flash_lock;
	retval = target_write_u32(target, APM32_FLASH_OBKEYR_B0, KEY2);
	if (retval != ERROR_OK)
		goto flash_lock;
		...
		unsigned int last)
{
	...
	for (unsigned int i = first; i <= last; i++) {
		retval = target_write_u32(target, apm32x_get_flash_reg(bank, APM32_FLASH_CR2), FLASH_PER);
		if (retval != ERROR_OK)
			goto flash_lock;
		...
		retval = target_write_u32(target,
				apm32x_get_flash_reg(bank, APM32_FLASH_CR2), FLASH_PER | FLASH_STRT);
		if (retval != ERROR_OK)
			goto flash_lock;
...
flash_lock:
	{
		int retval2 = target_write_u32(target, apm32x_get_flash_reg(bank, APM32_FLASH_CR2), FLASH_LOCK);
		if (retval == ERROR_OK)
			retval = retval2;
	}
	return retval;
		...
		uint32_t offset, uint32_t count)
{
	...
	retval = target_write_u32(target, apm32x_get_flash_reg(bank, APM32_FLASH_CR2), FLASH_PG);
	if (retval != ERROR_OK)
		goto reset_pg_and_lock;
...
reset_pg_and_lock:
	retval2 = target_write_u32(target, apm32x_get_flash_reg(bank, APM32_FLASH_CR2), FLASH_LOCK);
	if (retval == ERROR_OK)
		retval = retval2;
...
static int apm32x_probe(struct flash_bank *bank)
{
	...
	apm32x_info->probed = false;
	apm32x_info->register_base = FLASH_REG_BASE;
	apm32x_info->user_data_offset = 3;
...
static int apm32x_mass_erase(struct flash_bank *bank)
{
	...
	retval = target_write_u32(target, apm32x_get_flash_reg(bank, APM32_FLASH_CR2), FLASH_MER);
	if (retval != ERROR_OK)
		goto flash_lock;
	retval = target_write_u32(target, apm32x_get_flash_reg(bank, APM32_FLASH_CR2),
...
static const struct command_registration apm32f00x_exec_command_handlers[] = {
	{
		.name = "lock",
...
static const struct command_registration apm32f00x_command_handlers[] = {
	{
		.name = "apm32f00x",
		.mode = COMMAND_ANY,
		.help = "apm32f00x flash command group",
		.usage = "",
		.chain = apm32f00x_exec_command_handlers,
	},
	COMMAND_REGISTRATION_DONE
};

const struct flash_driver apm32f00x_flash = {
	.name = "apm32f00x",
	.commands = apm32f00x_command_handlers,
